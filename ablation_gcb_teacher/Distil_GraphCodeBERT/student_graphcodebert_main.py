...
def test(args, model, tokenizer, test_dataset, best_beta=None):
    # build dataloader
...
def main():
    ...
    gcb_model = Model(encoder=pretrained_model, tokenizer=tokenizer, config=pretrained_model.config, num_labels=len(cwe_label_map), args=args)
    teacher_model = TeacherModel(shared_model=gcb_model, tokenizer=tokenizer, num_labels=len(cwe_label_map), args=args)
    teacher_model.load_state_dict(torch.load("./saved_models/checkpoint-best-acc/gcbteacher.bin", map_location=args.device))
    ...
    if args.do_test:
        ...
        y_trues, y_preds = test(args, model, tokenizer, test_dataset, best_beta=args.beta)
