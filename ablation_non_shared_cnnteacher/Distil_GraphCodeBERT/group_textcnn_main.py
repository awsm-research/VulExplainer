...
from transformers import (AdamW, get_constant_schedule, RobertaConfig, RobertaTokenizer, RobertaModel)
...
def train(args, train_dataset, model, tokenizer, eval_dataset, cwe_label_map):
    ...
    #logit_adjustment = compute_adjustment(tau=1.2, args=args, cwe_label_map=cwe_label_map)
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            loss, contrastive_loss = model(input_ids=input_ids, 
                                           labels=labels, 
...
def main():
    ...
    parser.add_argument("--load_model_from_checkpoint", action='store_true',
                        help="")
    ...
    # group labels
    with open("./group_label_map.pkl", "rb") as f:
    ...
    if args.load_model_from_checkpoint:
        checkpoint_prefix = 'checkpoint-best-acc/group.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix)) 
